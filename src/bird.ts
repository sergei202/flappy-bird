...
export class Bird {
	...
	constructor(private startPos:P5.Vector) {
	...
	draw() {
		p5.push();
		if(this.alive) {
			if(this.player) p5.fill(0,0,255);
	...
	update() {
		...
		if(this.player && p5.keyIsDown(32)) this.flap();
		...
		if(this.pipeDist && !this.player) {
			...
			if(actions[0]>0.5) this.flap();
		}
	}

	checkCollidePipes(pipes:Pipe[]) {
		var collide = pipes.some(p => this.checkCollidePipe(p));
		if(collide) this.alive = false;
		return collide;
	}

	checkCollidePipe(pipe:Pipe) {
